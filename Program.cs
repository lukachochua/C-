    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                Console.WriteLine($"\nChecking data for {ourAnimals[i, 0]}");


                string ageStr = ourAnimals[i, 2].Replace("Age: ", "");
                if (ageStr == "?")
                {
                    bool validAge = false;
                    while (!validAge)
                    {
                        Console.WriteLine("The age for this pet is unknown.");
                        Console.WriteLine("Please enter a valid numeric age or type '?' if the age is still unknown:");
                        readResult = Console.ReadLine();
                        if (readResult != null)
                        {
                            if (readResult == "?")
                            ...
                            else if (int.TryParse(readResult, out petAge) && petAge >= 0 && petAge <= 25)
                            {
                ...
                else if (!int.TryParse(ageStr, out petAge) || petAge < 0 || petAge > 25)
                {
                    bool validAge = false;
                    while (!validAge)
                    {
                        Console.WriteLine("The age for this pet is invalid.");
                        Console.WriteLine("Please enter a valid numeric age between 0 and 25:");
                        readResult = Console.ReadLine();
                        if (readResult != null && int.TryParse(readResult, out petAge) && petAge >= 0 && petAge <= 25)
                        {
                            ourAnimals[i, 2] = "Age: " + petAge;
                            validAge = true;
                        }
                        ...
                        {
                            Console.WriteLine("Invalid age. Please enter a number between 0 and 25.");
                        }
                    }
                }

                string physicalDescription = ourAnimals[i, 4].Replace("Physical description: ", "").Trim();
                if (string.IsNullOrEmpty(physicalDescription) || physicalDescription.Length == 0)
                {
                    bool validDescription = false;
                    while (!validDescription)
                    {
                        Console.WriteLine("The physical description for this pet is invalid or incomplete.");
                        Console.WriteLine("Please enter a valid physical description:");
                        readResult = Console.ReadLine();
                        if (readResult != null && !string.IsNullOrEmpty(readResult.Trim()))
                        {
            ...
            Console.WriteLine("All data requirements are met. Press Enter to continue.");
            Console.ReadLine();
